...
function mission(){
  ...
  if(mod==="gps"){
    ...
    body=`<span class="kicker">${m.title}</span>
      ...
      <div class="dialogue"><strong>📍 Elegí la ubicación del árbol.</strong><br>Podés usar tu ubicación actual o tocar directamente el mapa si el árbol está más lejos.</div>

      <div class="tree-map-wrap">
        <div id="treeMap" aria-label="Mapa para seleccionar la ubicación del árbol"></div>
        <div class="map-help">Tocá el mapa o arrastrá el marcador hasta el lugar exacto.</div>
      </div>
...
function treeMarkerIcon(){
  return L.divIcon({
    ...
    html:`<div class="tree-marker-pin"><span>🌳</span></div>`,
    iconSize:[38,38],
    iconAnchor:[19,38]
  });
}
...
function setMapPoint(lat,lng,source="map",accuracy=null,center=false){
  ...
  if(center) locationMap.setView([lat,lng],18);
...
function refreshLeafletSize(){
  ...
  setTimeout(()=>locationMap?.invalidateSize({pan:false}),100);
  setTimeout(()=>locationMap?.invalidateSize({pan:false}),350);
  setTimeout(()=>locationMap?.invalidateSize({pan:false}),800);
}
...
function initLocationMap(){
  ...
  const center=existing
    ? [Number(existing.y),Number(existing.x)]
    : [-23.4425,-58.4438];

  const zoom=existing ? 17 : 6;
  ...
  if("ResizeObserver" in window){
    ...
    container._mapResizeObserver=observer;
  }

  window.addEventListener("resize",refreshLeafletSize,{passive:true});
}
    ...
    document.getElementById("gpsBtn").addEventListener("click",()=>{
      ...
      navigator.geolocation.getCurrentPosition(
        p=>{
          ...
          msg("Ubicación encontrada. Podés ajustar el marcador.");
        },
        err=>{
          failSound();
