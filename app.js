...
/* =========================================================
   AUTENTICACIÓN ARCGIS ONLINE — OAuth 2.0 + PKCE
   No usa ni expone el secreto de cliente.
========================================================= */

const AUTH_KEY = "mc_arcgis_auth";
const PKCE_KEY = "mc_arcgis_pkce";
const AUTH_CONTINUE_KEY = "mc_arcgis_continue";
...
function tokenStillValid(auth, marginMs=60000){
  return !!(auth?.accessToken && auth?.expiresAt && auth.expiresAt - marginMs > Date.now());
}

function randomBase64Url(byteLength=64){
...
function oauthBase(){
  return `${C.portalUrl.replace(/\/$/,"")}/sharing/rest/oauth2`;
}

async function startArcGISLogin(continueToGame=true){
  ...
  if(continueToGame){
    sessionStorage.setItem(AUTH_CONTINUE_KEY, "game");
  }
  ...
  window.location.assign(`${oauthBase()}/authorize?${params.toString()}`);
}
...
async function exchangeAuthorizationCode(code, verifier){
  ...
  if(!response.ok || data.error || !data.access_token){
    const detail = data.error?.message || data.error_description || "No se pudo completar el inicio de sesión.";
...
async function getAccessToken(){
  const auth = authData();
  if(tokenStillValid(auth)) return auth.accessToken;
  return await refreshArcGISToken();
}
...
async function handleOAuthCallback(){
  ...
  if(oauthError){
    const description = url.searchParams.get("error_description") || "ArcGIS canceló el inicio de sesión.";
  ...
  if(!pkce?.verifier || !pkce?.state || pkce.state !== returnedState){
    throw new Error("No pudimos validar el inicio de sesión de ArcGIS.");
  }
...
function intro(){
  ...
  bind(resetGame,async ()=>{
    const token = await getAccessToken();

    if(!token){
      await startArcGISLogin(true);
      return;
    }

    flow.page=save.ready?"camp":"register";
    render();
  });
}
...
function mission(){
  ...
  if(mod==="gps"){
    ...
    body=`<span class="kicker">${m.title}</span>
      <h2 class="section-title">¿Dónde está el Coloso?</h2>
      <div class="dialogue"><strong>📍 Elegí la ubicación del árbol.</strong><br>Usá tu ubicación actual para orientarte. Tu avatar muestra dónde estás y el árbol 🌳 marca el punto que se enviará.</div>

      <div class="tree-map-wrap">
        <div id="treeMap" aria-label="Mapa para seleccionar la ubicación del árbol"></div>
        <div class="map-help">Tocá el mapa o arrastrá el árbol 🌳. Los nombres, caminos y límites te ayudan a ubicarte.</div>
      </div>

      <div class="location-actions">
        <button id="gpsBtn" class="btn blue">📍 Usar mi ubicación actual</button>
      </div>

      <div id="locationSummary">
        ${coords ? locationSummaryHtml(coords) : `<div class="location-empty"><strong>Todavía no elegiste un punto.</strong><br><span class="small">Usá el GPS o tocá el mapa.</span></div>`}
      </div>

      ${companion(coords?"¡Perfecto! Podés mover el marcador si necesitás ajustar el punto.":"Marcá el lugar exacto donde está el árbol.")}`;
  }
  ...
  bindMission(mod);
  if(mod==="gps") requestAnimationFrame(()=>initLocationMap());
}


function locationSummaryHtml(coords){
  const source = coords.source === "gps" ? "Ubicación obtenida por GPS" : "Punto marcado en el mapa";
  const precision = Number.isFinite(coords.accuracy)
    ? `<span>Precisión aproximada: ${Math.round(coords.accuracy)} m</span>`
    : `<span>Ubicación elegida manualmente</span>`;

  return `<div class="location-selected">
    <strong>✅ ${source}</strong>
    ${precision}
    <span class="small">Latitud: ${Number(coords.y).toFixed(6)}</span>
    <span class="small">Longitud: ${Number(coords.x).toFixed(6)}</span>
  </div>`;
}

function updateLocationSummary(){
  const box=document.getElementById("locationSummary");
  if(box && flow.data.coords) box.innerHTML=locationSummaryHtml(flow.data.coords);
}

function createMarkerElement(className,content,label){
...
function setMapPoint(lat,lng,source="map",accuracy=null,center=false){
  flow.data.coords={x:Number(lng),y:Number(lat),accuracy:Number.isFinite(accuracy)?accuracy:null,source};

  if(!treeMarker){
    const treeElement=createMarkerElement(
      "tree-map-marker",
      `<div class="tree-map-marker-pin"><span>🌳</span></div>`,
      "Ubicación seleccionada del árbol"
    );

    treeMarker=new maplibregl.Marker({element:treeElement,draggable:true,anchor:"bottom"})
      .setLngLat([lng,lat])
      .addTo(locationMap);

    treeMarker.on("dragend",()=>{
      const point=treeMarker.getLngLat();
      setMapPoint(point.lat,point.lng,"map",null,false);
    });
  }else{
    treeMarker.setLngLat([lng,lat]);
  }

  if(center){
    locationMap.easeTo({center:[lng,lat],zoom:14,duration:900});
  }
...
function setPlayerPoint(lat,lng){
  if(!locationMap) return;
  if(!playerMarker){
    const avatar=companionData();
    const playerElement=createMarkerElement(
      "player-map-marker",
      `<div class="player-map-avatar">${avatar.icon}</div><div class="player-map-label">Vos</div>`,
      `Ubicación actual de ${save.player}`
    );
    playerMarker=new maplibregl.Marker({element:playerElement,anchor:"bottom"})
      .setLngLat([lng,lat])
      .addTo(locationMap);
  }else{
...
function initLocationMap(){
  ...
  if(typeof maplibregl==="undefined"){
    container.innerHTML=`<div class="map-load-error">No se pudo cargar el mapa. Recargá la página.</div>`;
  ...
  const existing=flow.data.coords;
  const initialCenter=existing ? [Number(existing.x),Number(existing.y)] : [-58.4438,-23.4425];

  locationMap=new maplibregl.Map({
    container:"treeMap",
    style:"https://tiles.openfreemap.org/styles/liberty",
    center:initialCenter,
    zoom:existing ? 14 : 5.5,
    minZoom:4,
    maxZoom:19,
    attributionControl:true
  });

  locationMap.addControl(new maplibregl.NavigationControl({showCompass:false,visualizePitch:false}),"top-left");

  locationMap.on("load",()=>{
    locationMap.resize();
    if(existing){
      setMapPoint(Number(existing.y),Number(existing.x),existing.source||"map",existing.accuracy,false);
    }
  });

  locationMap.on("click",event=>{
    setMapPoint(event.lngLat.lat,event.lngLat.lng,"map",null,false);
  });

  locationMap.on("error",event=>{
    console.error("Error del mapa:",event?.error||event);
  });

  requestAnimationFrame(()=>locationMap?.resize());
  setTimeout(()=>locationMap?.resize(),150);
  setTimeout(()=>locationMap?.resize(),500);
}
...
function locatePlayer(){
  ...
  navigator.geolocation.getCurrentPosition(
    position=>{
      ...
      setPlayerPoint(lat,lng);
      if(!flow.data.coords){
        setMapPoint(lat,lng,"gps",position.coords.accuracy,false);
      }
      locationMap?.easeTo({center:[lng,lat],zoom:14,duration:1000});
      if(button){
        button.disabled=false;
        button.textContent="🎯 Volver a mi ubicación";
      }
      msg("Ubicación encontrada. El avatar muestra dónde estás.");
    },
    error=>{
      failSound();
      const reason=error.code===1?"Permití el acceso a la ubicación.":error.code===3?"El GPS tardó demasiado. Probá otra vez.":"No pudimos obtener la ubicación.";
      if(button){
        button.disabled=false;
...
async function compressImage(file, maxBytes=1500*1024, maxDimension=2000){
  ...
  try{
    ...
    try{
      ...
      throw new Error("Esta imagen no se puede comprimir en este dispositivo. Probá con una foto JPG.");
    }
  }
  ...
  async function renderBlob(w,h,quality){
  ...
  while(blob.size > maxBytes && Math.max(width,height) > 1100){
...
function bindMission(mod){
  if(mod==="gps"){
    document.getElementById("gpsBtn")?.addEventListener("click",locatePlayer);
  }
  ...
  document.querySelectorAll("[data-file]").forEach(input=>{
    input.addEventListener("change",async ()=>{
      ...
      if(file.size > 30*1024*1024) return msg("La foto supera 30 MB.");

      const key=input.dataset.file;

      try{
        msg("Preparando la foto...");
        const prepared = await compressImage(file);
        flow.data.files[key]=prepared;
        ...
        console.error(error);
        failSound();
        msg(error.message || "No se pudo preparar la foto.");
      }
    });
  });
...
async function postForm(url, body, retryAuth=true){
  let token = await getAccessToken();

  if(!token){
    throw new Error("Tu sesión de ArcGIS venció. Volvé al campamento e iniciá sesión nuevamente.");
  }
  ...
  if((data?.error?.code===498 || data?.error?.code===499) && retryAuth){
    const refreshed = await refreshArcGISToken();

    if(refreshed){
      body.set("token",refreshed);
  ...
  if(!response.ok || data.error){
    const detail=data.error?.details?.join(" ") || data.error?.message || `Error HTTP ${response.status}`;
    throw new Error(detail);
  }
...
async function sendMission(){
  ...
  if(!token){
    flow.error="Tu sesión de ArcGIS venció. Cerrá esta misión, volvé a entrar al juego e iniciá sesión otra vez.";
    flow.page="sending";
    failSound();
    render();
    return;
  ...
  try{
    ...
    // Si addFeatures ya funcionó en un intento anterior, no lo repite.
    // Así un fallo en una foto no crea otro registro duplicado.
    let objectId=flow.createdObjectId;

    if(!objectId){
      objectId=await addFeature(feature);
      flow.createdObjectId=objectId;
    }

    const files=Object.entries(flow.data.files);
    let uploaded=Object.keys(flow.uploadedAttachments || {}).length;

    for(const [key,file] of files){
      if(flow.uploadedAttachments?.[key]) continue;

      await addAttachment(objectId,key,file);

      if(!flow.uploadedAttachments){
        flow.uploadedAttachments={};
      }

      flow.uploadedAttachments[key]=true;
      uploaded++;

      const status=document.getElementById("uploadStatus");
      if(status){
        status.textContent=`Fotos cargadas: ${uploaded}/${files.length}`;
      }
    }
...
function finish(){
  ...
  shell(
    `<div class="finish">
      ...
      <div class="small">Registro: ${flow.sentObjectId}</div>
...
async function boot(){
  try{
    const returnedFromLogin = await handleOAuthCallback();

    if(returnedFromLogin){
      const continueToGame = sessionStorage.getItem(AUTH_CONTINUE_KEY) === "game";
      sessionStorage.removeItem(AUTH_CONTINUE_KEY);
    ...
    console.error(error);
    setTimeout(()=>msg(error.message || "No se pudo iniciar sesión con ArcGIS."),100);
  }
