...
function initLocationMap(){
  ...
  const center=existing ? [existing.y,existing.x] : [-23.4425,-58.4438];
  ...
  locationMap.on("click",event=>{
    setMapPoint(event.latlng.lat,event.latlng.lng,"map",null,false);
  });

  if(existing){
    setMapPoint(existing.y,existing.x,existing.source||"map",existing.accuracy,false);
  }
